...
class ShadowAIDetector:
    ...
    def classify_risk(self, indicators_count: int, severity_score: float) -> RiskClassification:
    ...
    def detect_shadow_ai(
        self, file_path: str, patterns: Optional[List[str]] = None, confidence_threshold: float = 0.7
    ) -> Optional[ShadowAIFinding]:
        ...
        try:
            ...
            logger.info("shadow_ai_scan_started", extra={"file": file_path, "threshold": confidence_threshold})
            ...
            if finding.confidence >= confidence_threshold:
                ...
                logger.warning(
                    "shadow_ai_detected",
                    extra={"location": file_path, "risk": finding.risk_level, "confidence": finding.confidence},
                )
                return finding
    ...
    def get_findings_by_risk(self, risk_level: RiskClassification) -> List[ShadowAIFinding]:
        ...
        logger.info("findings_retrieved", extra={"risk_level": risk_level, "count": len(filtered)})
        return filtered
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "findings": [
                {"id": f.finding_id, "location": f.location, "risk": f.risk_level.value, "confidence": f.confidence}
                for f in self.findings
            ],
        }

        logger.info("shadow_ai_summary_generated", extra={"total_findings": len(self.findings)})
