...
AI_DISCLOSURE_PATTERNS = [
    ...
    r"ai_disclosure", r"disclose_ai", r"identify_as_ai",
    r"system_prompt.*?AI", r"assistant_message.*?AI",
    r"AI_DISCLAIMER", r"AI_IDENTITY",
]

# Capability documentation patterns
CAPABILITY_DOC_PATTERNS = [
    r"capabilities\s*=", r"limitations\s*=", r"model_capabilities",
    r"system_capabilities", r"describe_capabilities",
    r"model_card", r"system_card", r"CAPABILITIES", r"LIMITATIONS",
]

# Instructions-for-use patterns — user-facing guidance
INSTRUCTIONS_PATTERNS = [
    r"INSTRUCTIONS_FOR_USE", r"user_guide", r"USER_GUIDE",
    r"how_to_use", r"usage_instructions", r"operator_instructions",
]

# Provider identity patterns
PROVIDER_IDENTITY_PATTERNS = [
    r"provider_name", r"provider_identity", r"system_provider",
    r"organization\s*[=:]", r"vendor\s*[=:]",
    r"contact_info", r"support_email", r"provider_contact",
]

# Output interpretation / confidence patterns
OUTPUT_INTERPRETATION_PATTERNS = [
    r"confidence_score", r"confidence_level", r"uncertainty",
    r"prediction_confidence", r"response_confidence",
    r"interpret_output", r"explain_output", r"rationale",
    r"reasoning_trace", r"chain_of_thought",
]
...
def _check_ai_disclosure(file_contents: dict) -> TransparencyFinding:
    ...
    if hits:
        return TransparencyFinding(
            article=13, name="AI disclosure to users",
            status="pass",
...
def _check_capability_docs(file_contents: dict, scan_path: str) -> TransparencyFinding:
    ...
    if has_doc:
        return TransparencyFinding(
            article=13, name="Capability and limitation documentation",
            status="pass",
...
def _check_instructions_for_use(scan_path: str) -> TransparencyFinding:
    ...
    doc_files = [
        "OPERATOR_GUIDE.md", "USER_GUIDE.md", "INSTRUCTIONS.md",
        "RUNBOOK.md", "USAGE.md", "HOW_TO_USE.md",
    ]
    found = [f for f in doc_files if os.path.exists(os.path.join(scan_path, f))]
    if found:
        return TransparencyFinding(
            article=13, name="Instructions for use",
            status="pass",
...
def _check_provider_identity(file_contents: dict, scan_path: str) -> TransparencyFinding:
    ...
    if code_hits and has_doc:
        return TransparencyFinding(
            article=13, name="Provider identity disclosure",
            status="pass",
...
def _check_output_interpretation(file_contents: dict) -> TransparencyFinding:
    ...
    if hits:
        return TransparencyFinding(
            article=13, name="Output interpretation support",
            status="pass",
...
def _check_change_logging(scan_path: str) -> TransparencyFinding:
    ...
    if has_changelog:
        return TransparencyFinding(
            article=13, name="Change logging and versioning",
            status="pass",
