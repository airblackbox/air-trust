...
def _check_consent_management(file_contents: dict, scan_path: str) -> List[CodeFinding]:
    """GDPR Art. 6/7: Lawful basis and consent management."""
    strong_patterns = [
        r"consent_manage",
        r"lawful_basis",
        r"legal_basis",
        r"data_subject_consent",
        r"opt_in",
        r"opt_out",
        r"consent_record",
        r"withdraw_consent",
    ...
    strong_hits = [fp for fp, c in file_contents.items() if re.search(strong, c, re.IGNORECASE)]
    moderate_hits = [
        fp for fp, c in file_contents.items() if re.search(moderate, c, re.IGNORECASE) and fp not in strong_hits
    ]
...
def _check_data_minimization(file_contents: dict, scan_path: str) -> List[CodeFinding]:
    ...
    patterns = [
        ...
        r"field_filter",
        r"allowed_fields",
        r"required_fields_only",
        r"redact_unnecessary",
        r"purpose_limitation",
        r"collect_only",
        r"data_scope",
    ]
    combined = "|".join(patterns)
    hits = [fp for fp, c in file_contents.items() if re.search(combined, c, re.IGNORECASE)]
    if hits:
        return [
            CodeFinding(
                article=5,
...
def _check_right_to_erasure(file_contents: dict, scan_path: str) -> List[CodeFinding]:
...
def _check_data_retention(file_contents: dict, scan_path: str) -> List[CodeFinding]:
...
def _check_cross_border_transfer(file_contents: dict, scan_path: str) -> List[CodeFinding]:
    ...
    patterns = [
        ...
        r"eu_only",
        r"region_lock",
        r"geographic_restrict",
        r"transfer_impact",
        r"adequacy_decision",
        r"standard_contractual",
        r"scc",
        r"binding_corporate",
        r"data_localiz",
        r"sovereign",
    ...
    region_patterns = [
        r"region\s*=\s*['\"]eu",
        r"location\s*=\s*['\"]europe",
        r"AWS_REGION.*eu-",
        r"AZURE.*europe",
    ]
    region_combined = "|".join(region_patterns)
    region_hits = [
        fp for fp, c in file_contents.items() if re.search(region_combined, c, re.IGNORECASE) and fp not in hits
    ]
    if hits:
        return [
            CodeFinding(
                article=44,
                name="GDPR cross-border transfer safeguards",
                status="pass",
...
def _check_dpia_patterns(file_contents: dict, scan_path: str) -> List[CodeFinding]:
    """GDPR Art. 35: Data Protection Impact Assessment."""
    patterns = [
        r"dpia",
        r"impact_assessment",
        r"privacy_impact",
        r"risk_assessment.*data",
...
def _check_processing_records(file_contents: dict, scan_path: str) -> List[CodeFinding]:
    """GDPR Art. 30: Records of processing activities."""
    patterns = [
        r"processing_record",
        r"processing_activit",
        r"data_register",
        r"processing_log",
        r"record_of_processing",
        r"ropa",
        r"data_inventory",
...
def _check_breach_notification(file_contents: dict, scan_path: str) -> List[CodeFinding]:
