...
def generate_pdf(bundle: dict, output_path: str = "AIR_Blackbox_Compliance_Report.pdf") -> str:
    ...
    def badge(s):
        m = {
            "pass": ('<font color="#22c55e"><b>✓ PASS</b></font>', cell_s),
    ...
    fix_rows = [
        ...
        [
            P("1"),
            P("Set TRUST_SIGNING_KEY in .env"),
            P("Art. 12"),
    ...
    at_rows = [
        ...
        ["Chain", "No signing key set" if not audit.get("chain_valid") else "INTACT", "Storage", "Local"],
    ]
    ...
    fp_rows = [
        ...
        [
            ...
            P(
                "Cache TTL is not token expiry. Was generating false passes. Replaced with: max_agent_steps, execution_timeout, session_timeout."
            ),
        ],
        [
            ...
            P(
                "Haystack uses is_allowed for object serialization, not access control. Removed. Added human_in_the_loop/policies and confirmation_policy instead."
            ),
        ],
    ]
    ...
    story.append(
        Paragraph(
            ...
            S("sh2", fontSize=9, fontName="Helvetica-Bold", textColor=_c(NAVY), spaceBefore=6, spaceAfter=4),
        )
    )
    ...
    story.append(
        Paragraph(
            ...
            S("sh3", fontSize=9, fontName="Helvetica-Bold", textColor=_c(NAVY), spaceBefore=6, spaceAfter=4),
        )
    )
    ...
    vfy_rows = [
        ...
        [
            P("2"),
            P("air-blackbox replay --verify"),
            P(
                "Verifies the HMAC-SHA256 audit chain integrity. If any record was tampered with after the scan, the chain breaks and this command reports it."
            ),
        ],
        [
            P("3"),
            P("air-blackbox discover"),
            P(
                "Independently generates the AI-BOM (model inventory). Cross-check the models listed in the audit trail above against what your team knows is deployed."
            ),
        ],
        [
            P("4"),
            P("github.com/airblackbox/gateway"),
            P(
                "All scanner patterns are open source. Review sdk/air_blackbox/compliance/code_scanner.py to see exactly which regex patterns correspond to each check in this report."
            ),
        ],
    ]
